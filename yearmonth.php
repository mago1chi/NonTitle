...
<?php
	...
	if($tnum < 5){
		...
		where a.id=b.id and year=$year and month=$mon order by a.id desc";
		/* クエリからデータ取得 */
		$result = $link->query($query);
		if(!$result)
			die('クエリが失敗しました．');
		/* 記事の表示 */
		echo "<p id = \"category\">".$year."年".$mon."月の記事一覧</p>";
		...
		/* テーブルへのクエリ作成 */
		$query = "select a.id,a.title,a.content,a.timeStamp,b.name from article as a, tag as b 
		where a.id=b.id and year=$year and month=$mon order by a.id desc limit $num*5, 5";
		/* クエリからデータ取得 */
		$result = $link->query($query);
		if(!$result)
			die('クエリが失敗しました．');
		...
		echo "<b>";
		if($num > 0)
			echo "<a href = './yearmonth.php?year=$year&mon=$mon&num=",$num-1,"&tnum=$tnum'>&lt&ltprev</a>　";
		for($i = 0; $i < $pageNum; $i++){
			if($i == $pageNum-1){
				if($i == $num)
					echo $i+1;
				else
					echo "<a href = './yearmonth.php?year=$year&mon=$mon&num=$i&tnum=$tnum'>",$i+1,"</a>";
			}
			else{
				if($i == $num)
					echo $i+1,"　";
		...
		if($num < $pageNum-1)
			echo "<a href = './yearmonth.php?year=$year&mon=$mon&num=",$num+1,"&tnum=$tnum'>　next>></a>　";
		echo "</b>";
		echo "</center>";
	}
	echo "</section>";
